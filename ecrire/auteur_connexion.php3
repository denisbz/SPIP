...
echo "<br><br><br>";
gros_titre("$nom_auteur");

if (($connect_statut == "0minirezo") OR $connect_id_auteur == $id_auteur) {
	barre_onglets("auteur", "connexion");
}
...
function mySel($varaut,$variable) {
	...
	if ($connect_statut == "0minirezo" OR $connect_id_auteur == $id_auteur) {
		...
		echo "<FORM ACTION='auteur_connexion.php3?id_auteur=$id_auteur' METHOD='post'>";
			...
			if (mysql_num_rows($result_admin) == 0) {
				...
				while ($row_admin = mysql_fetch_array($result_admin)) {
					...
					if ($connect_toutes_rubriques AND $connect_id_auteur != $id_auteur) {
						echo " <font size=1>[<a href='auteurs_connexion.php3?id_auteur=$id_auteur&supp_rub=$id_rubrique'>supprimer cette rubrique</a>]</font>";
					}

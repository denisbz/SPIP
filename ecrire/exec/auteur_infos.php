...
function auteurs_interventions($auteur) {
	...
	AND autoriser('ecrire', '', '', $auteur)
		...
		echo afficher_ses_messages('<b>' . _T('info_discussion_cours') . '</b>', ", spip_auteurs_messages AS A, spip_auteurs_messages AS D", "A.id_auteur=$connect_id_auteur AND D.id_auteur = $id_auteur AND statut='publie' AND type='normal' AND rv!='oui' AND A.id_message=M.id_message AND D.id_message=M.id_message", $vus, false, false);
	
		echo afficher_ses_messages('<b>' . _T('info_vos_rendez_vous') . '</b>', ", spip_auteurs_messages AS A, spip_auteurs_messages AS D", "A.id_auteur=$connect_id_auteur AND D.id_auteur = $id_auteur AND statut='publie' AND type='normal' AND rv='oui' AND date_fin > ".sql_quote(date('Y-m-d H:i:s'))." AND A.id_message=M.id_message AND D.id_message=M.id_message", $vus, false, false);

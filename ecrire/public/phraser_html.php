...
define('CHAMP_ETENDU_RE', '/' . CHAMP_ETENDU . '/S');

define('BALISE_INCLURE','<INCLU[DR]E[[:space:]]*(\(([^)]*)\))?');
define('BALISE_INCLURE_RE', '/' . BALISE_INCLURE . '/S');
define('SQL_ARGS', '(\([^)]*\))');
...
function phraser_inclure($texte, $ligne, $result) {
	...
	while (preg_match(BALISE_INCLURE_RE, $texte, $match)) {
...
function phraser_idiomes($texte,$ligne,$result) {
	...
	while (preg_match("@<:(([a-z0-9_]+):)?([a-z0-9_]+)((\|[^:>]*)?:>)@i", $texte, $match)) {
		$p = strpos($texte, $match[0]);
		$debut = substr($texte, 0, $p);
...
function phraser_champs_interieurs($texte, $ligne, $sep, $result) {
	...
	while (true) {
		$j=$i;
		$n = $ligne;
		while (preg_match(CHAMP_ETENDU_RE, $texte, $match)) {
			$p = strpos($texte, $match[0]);
			$debut = substr($texte, 0, $p);
			if ($p) {
				$result[$i] = $debut;
		...
		if (preg_match(CHAMP_ETENDU_RE, $x))
			$texte = $x;
		else
	...
	// forcer {tout} pour avoir la rubrique mere...
			...
			elseif (($type == 'hierarchie') && ($param == 'id_rubrique'))
			  ...
			  if (preg_match('@^ *([0-9-]+) *(/) *(.+) *$@', $param, $m)) {
			    ...
			    $a2 = trim($m[7]);
			    if (preg_match("@^'.*'$@", $a2) OR preg_match('@^".*"$@', $a2))
			      $a2 = substr($a2,1,-1);

...
function nettoyer_chaine_indexation($texte) {
	$accents = chr(192).chr(193).chr(194).chr(195).chr(196).chr(197).
...
function deja_indexe($type, $id_objet) {
	...
	list($n) = @mysql_fetch_array(@mysql_query($query));
	return ($n > 0);
}
...
function indexer_objet($type, $id_objet, $forcer_reset = true, $full = true) {
	...
	case 'article':
		...
		$query2 = "SELECT spip_mots.* FROM spip_mots, spip_mots_articles AS lien WHERE lien.id_article=$id_objet AND spip_mots.id_mot=lien.id_mot";
		...
		$query3 = "SELECT spip_auteurs.* FROM spip_auteurs, spip_auteurs_articles AS lien WHERE lien.id_article=$id_objet AND spip_auteurs.id_auteur=lien.id_auteur";
	...
	if ($index) {
		...
		mysql_query("INSERT $table_index (hash, points, $col_id) VALUES ".join(',',$q));
	}
}
	...
	if ($visiter_sites == "oui") {
		...
		if ($result = mysql_query($query)) {
			while ($row = mysql_fetch_array($result)) {
				...
				mysql_query("UPDATE spip_syndic SET date_index=NOW() WHERE id_syndic=$id_syndic");
			}
		}
	}
}

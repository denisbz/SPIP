...
if ($text_article)
 ...
 "<a class='$c' href='" . http_php_scriptnq("articles","id_article=$id_article") . "'>$lang</a>";
		}
	}
...
echo "<form action='" . http_php_scriptnq("articles_tous","") . "' method='get'>";
...
function afficher_rubriques_filles($id_parent) {
	...
	if ($enfant[$id_parent]) {
	  while (list($id_rubrique, $titre) = each($enfant[$id_parent]) ) {
			...
			echo "<b class='verdana2'><a href='" . http_php_scriptnq("naviguer","id_rubrique=$id_rubrique") . "'>";
			echo $titre;
			echo "</b></a></div>\n";
				...
				if ($article[$id_rubrique]) {
					...
					while(list(,$zarticle) = each($article[$id_rubrique]) ) {
						...
						if ($text_article[$zarticle]["id_trad"] == 0 OR $text_article[$zarticle]["id_trad"] == $zarticle) {
							...
							echo "<a class='".$text_article[$zarticle]["statut"]."' href='" . http_php_scriptnq("articles","id_article=$zarticle") . "'>";

...
function affiche_progression_javascript($abs_pos) {
	...
	if ($abs_pos == '100 %') {
		...
		if ($GLOBALS['erreur_restauration'])
			echo "document.progression.recharge.value='".addslashes(unicode_to_javascript(_T('avis_erreur')))."';\n";
		else
			echo "document.progression.recharge.value='".addslashes(unicode_to_javascript(_T('info_fini')))."';\n";
	}

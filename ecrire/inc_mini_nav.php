		...
		while (list($i, $k) = each($ordre)) {
			...
			if ($le_parent == $id_rubrique) {
				...
				if ($rub[$i]["enfants"]) {
					...
					$url = generer_url_ecrire("ajax_page", "fonction=aff_rub&rac=$rac&exclus=$rub_exclus&id=$la_rub&col=".($col+1));
...
function mini_nav ($sel, $rac="",$fonction="", $rub_exclus=0, $aff_racine=false) {
	...
	$onClick .= "charger_id_url('" . generer_url_ecrire("ajax_page", "fonction=aff_rub&rac=$rac&exclus=$rub_exclus&id=0&col=1") . "', '".$rac."_col_1');";

...
if (!$etape) echo "<p><blockquote><font size=2>"._T('info_sauvegarde_echouee')." <a href=" . http_php_script("export_all","etape=1&gz=$gz") . ">"._T('info_procedez_par_etape')."</a></font></blockquote><p>";
...
if (!$etape OR $etape == 13){
	...
	echo "<p>"._T('info_sauvegarde_reussi_01')."</b><p>"._T('info_sauvegarde_reussi_02', array('archive' => $archive))." <a href=" . http_php_script("index","") . ">"._T('info_sauvegarde_reussi_03')."</a> "._T('info_sauvegarde_reussi_04')."\n";
}
else {
	$etape_suivante = $etape + 1;
	if ($debut_limit > 1) echo "<p align='right'> <a href=" . http_php_script("export_all","etape=$etape&debut_limit=$debut_limit&gz=$gz") . ">>>>> "._T('info_etape_suivante')."</a>";
	else echo "<p align='right'> <a href=" . http_php_script("export_all","etape=$etape_suivante&gz=$gz") . ">>>>> "._T('info_etape_suivante')."</a>";
}
install_fin_html();
